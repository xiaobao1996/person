...
import cn.hy.infoReport.common.utils.ResponseUtils;
...
public abstract class AbstractTicketValidationFilter extends AbstractCasFilter {
    ...
    public final void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse,
        ...
        if (CommonUtils.isNotBlank(ticket)) {
            ...
            String serviceName = servletRequest.getParameter("service");
            try {
...
//                }
                ...
                ResponseUtils.buildResponse(request, response);
                if (e.getMessage().contains("不符合目标服务")) {
                    response.getWriter().write(JSONObject.toJSONString(new Result(ResultCode.SYS_ERROR.getCode(), "TICKET不符合目标服务！", null)));
                } else {
                    response.getWriter().write(JSONObject.toJSONString(new Result(ResultCode.TICKET_EXPIRED.getCode(), "授权信息已过期，请重新登录！", SsoProperties.casServerLoginUrl)));
                }
